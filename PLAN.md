...
Legend: â˜ not started Â· â˜ in progress Â· â˜‘ done
...
- 10 min: concept reflection (â€œexplain it backâ€)
- 15 min: assessment (quiz + practical task + rubric)

---

## Module 0 â€” Development environment and language crash courses (no prior knowledge)

â˜‘ L00 VS Code development environment (for this course)
   - Install VS Code
...
â˜ L0B C crash course (for embedded SDK code)
   - compilation model, headers, functions
   - pointers (intro), arrays, structs
   - memory model mental picture (stack vs static vs heap)

â˜ L0C C++ crash course (minimal for Pico SDK)
   - what changes vs C
   - references vs pointers (conceptual)
   - constructors/destructors idea (conceptual)
   - â€œuse C first, C++ laterâ€ rule of thumb

---

## Module 1 â€” Pico foundations (MicroPython + Pico SDK)

â˜ L01 Setup & flashing (MicroPython + Pico SDK â€œhello worldâ€)
â˜ L02 GPIO output + input + pull-ups + debouncing
â˜ L03 Timing: delays vs timers + PWM basics
â˜ L04 UART logging + simple command console

â˜ L04A Nonvolatile storage (EEPROM equivalent on RP2040/RP2350)
...
## Module 2 â€” Communication and real devices

â˜ L05 I2C basics + error handling
   - scanning the bus
   - addressing
   - retry strategy and timeouts

â˜ L05A Temperature/Humidity/Pressure sensor (THP) via I2C
   ...
   - Assessment: implement â€œsensor healthâ€ checks (range validation + retry)

â˜ L05B Button matrix scanning (local keyboard)
...
â˜ L06 SPI throughput + display/ADC (choose 1 device)
â˜ L07 Interrupts: GPIO + timer + safe shared state

---

## Module 3 â€” RP â€œsuperpowersâ€

â˜ L08 PIO intro: waveform generator
â˜ L09 PIO: custom protocol (choose a target)
â˜ L10 DMA: continuous sampling into ring buffer

---

## Module 4 â€” Architecture & product skills

â˜ L11 Dual-core patterns: queues, producer/consumer
â˜ L12 Low power modes: compare RP2040-family vs RP2350-family boards
â˜ L13 USB device: CDC serial
â˜ L14 USB HID device (keyboard or mouse)
...
â˜ L15 Capstone build (choose one final project)
â˜ L16 Capstone polish: docs, tests, release tag
